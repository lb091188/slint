...
| `\\`            | `\`                                                                                             |
| `\n`            | 新的一行                                                                                        |
| `\u{x}`         | 其中 `x` 是一个十六进制数，表示由该数字代表的 Unicode 码点。 |
| `\{expression}` | 表达式求值的结果                                                         |

任何未转义的 `\` 后跟的其他字符都会导致错误。
